...
<StoryAndSource id="stories-nesting--scenario-1" />

## Portals

When `<ClickAwayListener>` elements are nested inside React Portals, they work the same way as if they weren't.

Even though the elements are not nested in DOM, they are nested in the React tree. Internally, we determine that the second layer is "inside" the first layer by checking whether React events propagate from the layer below and by looking at the React Context hiearchy established by the `<ClickAwayListener>` elements.
...
<StoryAndSource id="stories-nesting--scenario-2" />
